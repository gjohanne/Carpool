...
class loginWindow(Screen):
    ...
    def validate(self):
        ...
        users = pd.read_csv('CAPSTONE_Project\login.csv')
    ...
    def signupbtn(self):
        ...
        if self.email.text != "":
            if self.email.text not in users['Email'].unique():
                ...
                user.to_csv('CAPSTONE_Project\login.csv',
...
class logDataWindow(Screen):
    ...
    def get_gps_latitude(self):
        self.latitude = self.decimal_precision(
            0.01 * random.random() + 50.6394, DECIMAL_PRECISION)
        return self.latitude  # rounding

    def get_gps_longitude(self):
        self.longitude = self.decimal_precision(
            0.01 * random.random() + 50.6394, DECIMAL_PRECISION)
